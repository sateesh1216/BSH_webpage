  ...
  return (
    <>
      ...
      >
        ...
        <div className="mx-auto flex h-20 w-[92%] max-w-7xl items-center gap-3 lg:gap-4 xl:gap-8">
          {/* Logo */}
          ...
          {/* Desktop Navigation — compact at the tight lg tier (1024-1279px),
              roomier from xl (1280px+) once there's space to spare */}
          <nav className="hidden shrink-0 items-center gap-0.5 rounded-full border border-white/40 bg-white/30 p-1.5 shadow-inner lg:flex xl:gap-1">
            {navLinks.map((link) => {
              ...
              if (link.children) {
                return (
                  ...
                  >
                    <button className="flex items-center gap-1 whitespace-nowrap rounded-full px-2.5 py-2 text-[13px] font-semibold text-slate-700 transition hover:text-[#155EEF] xl:px-4 xl:text-sm">
                      {link.label}
                      <ChevronDown
                        size={16}
              ...
              return (
                <Link
                  key={link.label}
                  to={link.href}
                  className={`group relative whitespace-nowrap rounded-full px-2.5 py-2 text-[13px] font-semibold transition xl:px-4 xl:text-sm ${
          ...
          {/* Desktop Search Bar — flex-1 so it claims all real leftover space,
              with a hard min-width so it can never collapse to just its icons.
              The reserved slot has a fixed height; MobileSearchBar itself is
              absolutely positioned inside it so its dropdown/results panel
              (which expands in normal document flow) overlays the page below
              instead of stretching the header row and shifting everything else. */}
          <div className="hidden min-w-[170px] flex-1 lg:relative lg:block lg:h-12 xl:min-w-[280px]">
            <div className="lg:absolute lg:inset-x-0 lg:top-0">
              <MobileSearchBar />
            </div>
          </div>

          {/* Call Button — icon-only at the tight lg tier, full number from xl */}
          ...
          <button
            ...
            className={`ml-auto grid h-11 w-11 shrink-0 place-items-center rounded-full backdrop-blur-md transition-all duration-300 lg:hidden ${
              menuOpen ? "bg-[#155EEF] text-white" : "bg-white/60 text-black hover:bg-white/80"
            }`}
          >
